...
struct rx2_state {
    ...
    unsigned int mainOLlockDetect;
	unsigned int lfOLlockDetect;
	unsigned int scOLlockDetect;
...
static ssize_t rx2_ifout_gain_write(struct iio_dev *indio_dev,
		...
		if (ret)
		    return ret;
		mutex_lock(&indio_dev->mlock);
		if (readin < 0 || readin > 36)
		    ret = -EINVAL;
		else{
			code = gaincodes[(u8) readin]; //integer part of gain
			st->ifoutgain = code;
			st->ifingaindb = readin;
...
static const struct iio_chan_spec_ext_info rx2_ddc_ext_info[] = {
	...
	_RX2_EXT_INFO("dcfilter", 4, rx2_ddc_write, rx2_ddc_read), 		//8
    {},
};
